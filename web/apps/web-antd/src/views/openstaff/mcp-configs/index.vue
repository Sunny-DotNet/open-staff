...
// ===== 初始化 =====
onMounted(async () => {
  // 检查路由参数中是否有 serverId 过滤
  const sid = route.query.serverId as string | undefined;
  if (sid) filterServerId.value = sid;
  await fetchAll();
});
</script>

<template>
  <Page title="MCP 配置管理" description="管理所有 MCP 服务器的配置实例">
    <template #extra>
      <Space>
        <Select
